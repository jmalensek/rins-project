...
class greet_people(Node):
    ...
    def calculate_shortest_path(self):
        # rearange self.queue such that the navigation path between them is smallest
        points = list(self.queue[:self.n_persons])

        # Use robot current pose as the start if available; otherwise use the first detection.
        if self.rc.initial_pose_received and self.rc.current_pose is not None:
            start = (
                float(self.rc.current_pose.pose.position.x),
                float(self.rc.current_pose.pose.position.y),
            )
        else:
            start = points[0]
        ...
        best_order = min(itertools.permutations(points), key=path_length)
        self.queue = list(best_order)
        self.get_logger().info(f"Reordered detections for shortest path: {self.queue[:self.n_persons]}")
